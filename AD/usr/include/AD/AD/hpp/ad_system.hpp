/*
ad_system.hpp
The function of the system was banned, so I wrote an imitation.

Other platforms dedicated to iOS should also be available because it has not been tested.

It may be on the system-restricted platform or on the system. Our code only needs to be modified and can be used there.
...
// int ad_system(const std::string& command); The old version of the function only supports bash and was finally eliminated, so it is annotated in the memorial.
